"""
Module de stop words français pour l'API.

⚠️  Ce fichier est dupliqué dans ingest/src/ingest/stopwords.py
    Si vous modifiez ce fichier, pensez à mettre à jour l'autre aussi.
"""

# Liste complète des stop words français (version set pour des lookups rapides O(1))
FRENCH_STOP_WORDS = {
...
def normalize_query(query: str) -> str:
    """
    Normalise une requête de recherche en supprimant les stop words et la ponctuation.

    Cette fonction améliore la recherche Meilisearch en filtrant les mots non pertinents
    des requêtes en langage naturel (ex: "Quels sont les modes..." → "modes").

    Args:
        query: La requête utilisateur en langage naturel

    Returns:
        La requête nettoyée, prête pour Meilisearch
    """
    import re

    # Enlève la ponctuation et normalise les espaces
    clean_query = re.sub(r'[^\w\s]', ' ', query.lower())

    # Filtre les stop words
    words = [w for w in clean_query.split() if w and w not in FRENCH_STOP_WORDS]

    # Retourne la requête normalisée, ou la requête originale si tous les mots sont des stop words
